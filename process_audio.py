...
def transcribe_file(audio_path, output_directory):
    ...
    print(f"\nStep 1: Transcribing {os.path.basename(audio_path)}...")
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"✓ Transcription completed: {output_filename}")
...
def translate_json(json_path):
    ...
    try:
        ...
        print(f"✓ Translation completed and saved to {os.path.basename(json_path)}")
        return True
...
def main():
    parser = argparse.ArgumentParser(
        description="Process audio file: transcribe and translate",
    ...
    if not args.audio_file.lower().endswith(AUDIO_EXTENSIONS):
        print(f"Error: File '{args.audio_file}' is not a supported audio file.", file=sys.stderr)
    ...
    if not json_path:
        print("✗ Transcription failed. Stopping.", file=sys.stderr)
        sys.exit(1)
    
    # Step 2: Translate
    if not translate_json(json_path):
